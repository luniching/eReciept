...
*/
    function extend(dust) {
        ...
        /**
* Include a template with respect to the current flow.
* Attributes:
* - template (the name of the template to include, public/templates/{template}.dust)
* - * (arbitrary attributes will be passed into template as dust variables)
*
...
// get registry for specified directory
for (var i = 0; i < templateParts.length - 1; i++) {
registry = registry[templateParts[i]];
}

// check for flow specific files
while (flow) {
if (registry[templateParts[templateParts.length - 1] + "-" + flow + ".dust"]) {
...
return chunk.partial(template, context, params);
};
